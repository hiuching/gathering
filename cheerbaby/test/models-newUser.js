...
describe('user-models', function () {
	...
	var dummyUser1, dummyUser2, mergedDummy;
	...
	describe('create user', function () {
		...
		it("should update verification code when registering with existing email (not activated)", function (done) {
			var options = {};
			options.emails = [{
				"label" : "primary",
				"email" : dummyUser1.getPrimaryEmail()
			}];
			// User.createAndSendVerificationLink(options, function (err, newUser) {
			User.create(options, function (err, newUser) {
				should.not.exist(err);
				newUser.should.be.an.Object;
				newUser.verificationCode.code.should.not.equal(dummyUser1.verificationCode.code);
				done();
			});
		});

		it("should activate the dummy user account", function (done) {
			var options = {
				conditions : {
					id : dummyUser1._id
				},
			...
			User.updateById(options, function (err, updatedUser) {
				should.not.exist(err);
				updatedUser.isActivated.should.equal(true);
				done();
			});
		});

		it("should fail to create duplicate user (activated)", function (done) {
			var options = {};
			options.emails = [{
				"label" : "primary",
				"email" : dummyUser1.getPrimaryEmail()
	...
	describe('oauth2', function () {
		...
		it("should get token string by auth code", function (done) {
			var authCode = {
				"clientId" : 'GhWniizvcXe2pfBpm2bdgunz', // really existing clientId, be careful when deleting from DB
		...
		it("should fail to authenticate user by token", function (done) {
			var token = {};
		...
		it("should authenticate user by token string", function (done) {
	...
	describe('merge dummy users', function () {
		it("should save merged user and deactive users", function (done) {
			var mergerOptions = {};
			mergerOptions.emails = dummyUser1.emails;
			mergerOptions.previousUserIds = [dummyUser1._id, dummyUser2._id];
			mergerOptions.isMerge = true;
			User.saveMergedUserAndDeactivePreviousUsers(mergerOptions, function (err, savedUser) {
				should.not.exist(err);
				savedUser.previousUserIds.should.be.an.Array;
				savedUser.previousUserIds.length.should.equal(2);
				mergedDummy = savedUser;
				User.findUsersByIds({userIds: savedUser.previousUserIds}, function (err, previousUsers) {
					should.not.exist(err);
					previousUsers.should.be.an.Array;
					previousUsers.length.should.equal(mergerOptions.previousUserIds.length);
					previousUsers.forEach(function (previousUser) {
						previousUser.isActive.should.equal(false);
					});
					done();
				});
			});
		...
		// it("should save merged user, deactive users and send verification link", function (done) {
			// var mergerOptions = {};
			// mergerOptions.emails = dummyUser1.emails;
			// mergerOptions.previousUserIds = [dummyUser1._id, dummyUser2._id];
			// mergerOptions.isMerge = true;
			// User.saveMergedUserAndDeactivePreviousUsersAndSendVerificationLink(mergerOptions, function (err, savedUser) {
				// should.not.exist(err);
				// savedUser.previousUserIds.should.be.an.Array;
				// savedUser.previousUserIds.length.should.equal(2);
				// mergedDummy = savedUser;
	...
	describe('delete dummy users', function () {
		it("should remove the dummy users while finished testing", function (done) {
			// console.log(dummyUser1._id, dummyUser2._id);
			User.deleteUsers({_id: {$in: [dummyUser1._id, dummyUser2._id, mergedDummy._id]}}, function(err){
				should.not.exist(err);
				done();
			});
		});
